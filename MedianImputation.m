function table = MedianImputation(filename)
    ...
    for col = 1:length(numeric_column_names)
        ...
        % Menghitung mean dari data valid dan 
        % menggunakan mean sebagai imputation value
        valid_data = column_data(~isnan(column_data(1:last_valid_index)));
        imputation_value = median(valid_data);
    
        % Lakukan imputasi hanya pada missing values yang valid
        column_data(missing_indices) = imputation_value;
